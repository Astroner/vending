...
export const Display: FC<DisplayProps> = (props) => {
    ...
    const addSlot = useCallback(() => {
        setEditableInventory((prev) => {
            ...
            const freeSlot = new Array(Model.TOTAL_SLOTS)
                .fill(null)
                .map((_, i) => i + 1)
                .filter((slot) => !prev.find((item) => item.slot === slot))[0];
    ...
    return (
        ...
        >
            ...
            {cameraPosition !== "front" && (
                <div
                    className={cn.back}
                    onClick={() => view?.setCameraPosition("front")}
                ></div>
            )}

            <Coins
                isOpen={
                    (cameraPosition === "numpad" &&
                        modelState === "acceptingCoins") ||
                    isPocketVisible
                }
                coins={pocket}
                onSelect={selectCoin}
            ...
            <button
                className={cn.wallet}
                onClick={() => setIsPocketVisible((p) => !p)}
            >
                <WalletIcon />
            </button>
