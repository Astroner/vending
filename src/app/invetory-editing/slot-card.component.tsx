...
export const SlotCard: FC<SlotCardProps> = memo(({ onChange, ...props }) => {
    ...
    const colorChange = useCallback<ColorChangeHandler>((e) => {
    ...
    const changeItems = useCallback((count: number) => {
        onChange(props.index, {
            ...props.info,
            count
        })
    ...
    return (
        <div className={cn.slot} >
            <div className={cn.slot__number}>
                {props.info.slot}
            </div>
            <div className={cn.slot__content}>
                <div style={{ background: colorString }} className={cn.slot__color} onClick={() => setIsPickerOpen(true)}>
                    {isPickerOpen && (
                        <ClickAwayListener onClickAway={closePicker}>
                            <SketchPicker disableAlpha color={colorString} onChangeComplete={colorChange} className={cn.slot__picker} />
                        </ClickAwayListener>
                    )}
                </div>
                <div className={cn.slot__numerics}>
                    <div className={cn.slot__input}>
                        ...
                        <NumberInput min={1} max={props.maxItems} value={props.info.count} onChange={changeItems} />
                    </div>
                    <div className={cn.slot__input}>
                        <div className={cn.slot__input__label}>
                            Price
            ...
            <button title="Delete" className={cn.slot__delete} onClick={() => props.onDelete(props.index)}>
                x
            </button>
        </div>
    )
